...
const MEASURES = {
  population: {
    label: "人口", unit: "人", hasAge: true, sumOk: true,
    why: "age_class='total' と sex='total' の行を除外済みのため、SUMしても二重計上にならない",
  },
  households: {
    label: "世帯数", unit: "世帯", hasAge: false, sumOk: true,
    why: "世帯数は年齢・男女の内訳を持たず、1町丁1ヶ月あたり1行のみ（17,160行 = 88町丁 × 195ヶ月）",
  },
};
...
/* v_households_5y は列を絞って作られており、age_class / sex を持たない。
   observations_5y 側で households は total/total の1行のみ（17,160行 = 88×195）。
   ビューの時点で1町丁1ヶ月1行なので、追加の防御は不要かつ参照不可。 */
const EXTRA_WHERE = {};
